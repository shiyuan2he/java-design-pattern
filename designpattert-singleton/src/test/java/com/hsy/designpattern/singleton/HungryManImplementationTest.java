...
public class HungryManImplementationTest {
    private static final Logger _logger = LoggerFactory.getLogger(HungryManImplementationTest.class) ;
    @Test
    public void getInstance() throws Exception {
        for (int i = 0; i < 10000; i++) {
            FixedThreadPool.getInstince().getFixedThreadPool().submit(new Thread(() ->
                _logger.info("{}",HungryManImplementation.getInstance() == HungryManImplementation.getInstance())
            ));
        }
    }
}